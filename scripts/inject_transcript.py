...
Injects CSV-based transcript data into the Ember SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "Ember"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "Ember"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Ember"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "Ember"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Ember database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Ember has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Ember sidebar.")

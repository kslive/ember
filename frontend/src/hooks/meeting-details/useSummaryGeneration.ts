...
function extractTopicFromBody(md: string): string | null {
...
function buildFrontmatter(meeting: any, body: string): string {
...
async function autoExportSummaryToMd(meeting: any, markdown: string): Promise<void> {
  try {
    const store = await load('preferences.json');
    const folder = (await store.get('save_summary_folder')) as string | null;
    if (!folder) {
      console.log('[autoExport] save_summary_folder not set, skipping');
      return;
    }
    const body = stripLeadingFrontmatter(cleanLeakedMarkdown(markdown));
    const h1 = extractTopicFromBody(body);
    const d = meeting?.created_at ? new Date(meeting.created_at) : new Date();
    const fallbackName = `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}-${pad2(d.getHours())}-${pad2(d.getMinutes())}.md`;
    const fileName = h1
      ? `${h1.replace(/[\\/:*?"<>|]/g, '-').replace(/\s+/g, ' ').trim().slice(0, 120)}.md`
      : fallbackName;
    const content = buildFrontmatter(meeting, body) + body;
    const path = await invokeTauri<string>('save_summary_md', {
      folder, fileName, content,
    });
    console.log('[autoExport] Wrote summary to', path);
  } catch (e) {
    console.warn('[autoExport] Failed:', e);
  }
}
...
export function useSummaryGeneration({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = (await invokeTauri('api_get_summary', { meetingId: meeting.id })) as any;
        if (cancelled) return;
        const st = String(s?.status || '').toLowerCase();
        const active = st === 'processing' || st === 'pending' || st === 'summarizing' || st === 'regenerating';
        if (!active) return;

        setSummaryStatus('summarizing');
        startSummaryPolling(meeting.id, meeting.id, async (pollingResult: any) => {
          if (cancelled) return;
          const ps = String(pollingResult?.status || '').toLowerCase();
          if (ps === 'completed' && pollingResult.data) {
            const mn = pollingResult.data.MeetingName || pollingResult.meetingName;
            if (pollingResult.data.markdown) {
              const cleaned = cleanLeakedMarkdown(pollingResult.data.markdown);
              setAiSummary({ markdown: cleaned } as any);
              void autoExportSummaryToMd(meeting, cleaned);
            } else {
              setAiSummary(pollingResult.data);
            }
            setSummaryStatus('completed');
            if (mn) updateMeetingTitle(mn);
      ...
      startSummaryPolling(meeting.id, process_id, async (pollingResult) => {
        ...
        if (pollingResult.status === 'completed' && pollingResult.data) {
          ...
          if (pollingResult.data.markdown) {
            ...
            const cleanedMarkdown = cleanLeakedMarkdown(pollingResult.data.markdown);
            setAiSummary({ markdown: cleanedMarkdown } as any);
            setSummaryStatus('completed');

            void autoExportSummaryToMd(meeting, cleanedMarkdown);

...
impl AudioMixer {
    ...
    pub fn new(mixing_mode: MixingMode) -> Self {
        Self {
            rms_analyzer: RmsAnalyzer::new(1024),
            ducking_processor: DuckingProcessor::new(0.1, 0.01, 0.1),
            crossfade_processor: CrossfadeProcessor::new(256),
            mixing_mode,
    ...
    fn calculate_dynamic_ratios(&self, mic_rms: f32, system_rms: f32) -> (f32, f32) {
        ...
        let mic_ratio = (mic_rms / total_level).max(0.1).min(0.9);

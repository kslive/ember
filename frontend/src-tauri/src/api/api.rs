    ...
    match SettingsRepository::get_transcript_config(pool).await {
        Ok(Some(mut config)) => {
            ...
            if config.provider == "parakeet" {
                log_info!("Coercing legacy parakeet transcript config -> localWhisper");
                config.provider = "localWhisper".to_string();
                config.model = crate::config::DEFAULT_WHISPER_MODEL.to_string();
            }
            log_info!(
        ...
        Ok(None) => {
            log_info!("No transcript config found, returning whisper default.");
            Ok(Some(TranscriptConfig {
...
pub async fn api_save_transcript<R: Runtime>(
    ...
    if let Some(ref fp) = folder_path {
        if !fp.trim().is_empty() {
            match MeetingsRepository::find_id_by_folder_path(pool, fp).await {
                Ok(Some(existing_id)) => {
                    log_info!(
                        "api_save_transcript: dedup by folder_path — meeting already saved for '{}', returning {}",
                        fp, existing_id
                    );
                    return Ok(serde_json::json!({
                        "status": "success",
                        "message": "Duplicate save coalesced (folder_path)",
                        "meeting_id": existing_id,
                    }));
                }
                Ok(None) => {  }
                Err(e) => {
                    log_warn!("api_save_transcript: folder_path dedup lookup failed (continuing): {}", e);
                }
            }
        }
    }
    ...
    static LAST_SAVE: Mutex<Option<(u64, String, Instant)>> = Mutex::new(None);
    let fingerprint: u64 = {
        ...
        feed(&meeting_title);
        feed(&transcripts_to_save.len().to_string());
        if let Some(t) = transcripts_to_save.first() { feed(&t.text); }
        if let Some(t) = transcripts_to_save.last()  { feed(&t.text); }
        h
    };
    if let Ok(guard) = LAST_SAVE.lock() {
        if let Some((fp, prev_id, when)) = guard.as_ref() {
            if *fp == fingerprint && when.elapsed() < Duration::from_secs(30) {
                log_info!(
                    "api_save_transcript: dedup hit — returning existing meeting_id {}",
                    prev_id
                );
                return Ok(serde_json::json!({
                    "status": "success",
                    "message": "Duplicate save coalesced",
                    "meeting_id": prev_id,
                }));
            }
        }
    ...
    .await
    {
        Ok(meeting_id) => {
            ...
            if let Ok(mut guard) = LAST_SAVE.lock() {
                *guard = Some((fingerprint, meeting_id.clone(), Instant::now()));
            }
            Ok(serde_json::json!({
                "status": "success",

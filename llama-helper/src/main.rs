...
impl ModelState {
    ...
    fn generate(
        ...
        // Thread count: keep it conservative so the machine stays responsive
        // while the model runs. Leave at least 2 cores for the OS/UI; never
        // grab more than half the cores. On an M-series this means the system
        // doesn't freeze during generation.
        let threads: i32 = std::thread::available_parallelism()
            .map(|n| {
                let cores = n.get() as i32;
                (cores / 2).clamp(1, (cores - 2).max(1))
            })
            .unwrap_or(2);

        // n_batch was = context_size (up to 32k!). That makes llama.cpp allocate
        // a huge Metal compute buffer and spike memory hard during prompt
        // ingestion — the main cause of the whole Mac hanging on a 16 GB
        // machine. A fixed, modest batch processes the prompt in chunks with a
        // tiny, steady buffer. Prefill is marginally slower but the system
        // stays usable. (KV-cache size is governed by n_ctx, set per-model.)
        let batch: u32 = 512;

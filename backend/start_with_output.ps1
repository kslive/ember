...
Write-Host "====================================="
Write-Host "Ember Backend Startup"
Write-Host "====================================="
...
Write-Host "====================================="
Write-Host "Starting Ember Backend"
Write-Host "====================================="
...
# Check if ember-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for ember-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\ember-frontend\ember-frontend.exe",
    "$env:LOCALAPPDATA\Programs\ember\ember-frontend.exe",
    "$env:ProgramFiles\ember-frontend\ember-frontend.exe",
    "${env:ProgramFiles(x86)}\ember-frontend\ember-frontend.exe",
    "$env:APPDATA\ember-frontend\ember-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("ember-frontend.exe", "ember.exe")
...
if ($frontendInstalled) {
    Write-Host "Ember frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Ember frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Ember frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Ember frontend launched successfully."
        }
    }
} else {
    Write-Host "Ember frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Ember frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if ember is now installed and launch it

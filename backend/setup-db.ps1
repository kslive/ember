...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.ember\meeting_minutes.db",
        "$env:USERPROFILE\Documents\ember\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/ember-backend/*/backend/meeting_minutes.db"
    }

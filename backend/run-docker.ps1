...
$AppProjectName = "ember-backend"
$AppContainerName = "ember-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "ember-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server ember-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server ember-backend
...
function Invoke-LogsCommand {
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "ember-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, ember-backend"
        ...
        $logArgs += @("whisper-server", "ember-backend")
    }
